...
const MessageTopBar = ({ selectedGroup, setSelectedGroup }) => {
  ...
  const onSelectUserHandler = (user) => {
    ...
    const selectedUserIdx = existingSelectedUsers.findIndex(
      (i) => i.id === user.id
    );
        ...
        return (
          <Box className="max-h-fit flex flex-1 flex-col justify-between capitalize py-8">
            <div className="flex flex-col items-center gap-2">
              ...
              <Typography>
                Created -{" "}
                {format(parseISO(selectedGroup.createdAt), "dd-MM-yyyy")}
              </Typography>
            </div>
            <Button
              onClick={existFromGroupHandler}
              className="self-center hover:bg-slate-200"
              color="error"
            >
              Exit group
            </Button>
      ...
      case "members":
        return (
          <List className="flex flex-1 flex-col capitalize">
            {isAdminRef.current && (
              <ListItem
                onClick={() => setSelected("addUsers")}
                className="flex gap-4 border-b hover:bg-slate-100"
              >
              ...
              return (
                <React.Fragment key={user.userId}>
                  <ListItem
                    ...
                    onClick={(e) => {
                      isAdminRef.current && handleClick(e, user);
                    }}
          ...
          <List className="flex flex-col items-center pt-4 h-full overflow-y-auto scrollbar-thin scrollbar-thumb-emerald-300 scrollbar-track-slate-100">
            ...
            {users
              ...
              .map((user) => {
                ...
                const disable = group.some(
                  (isUser) => isUser.userId === user.id
                );
                return (
                  <ListItem
  ...
  return (
    <>
      <Box
        ...
        component="div"
        className="absolute flex justify-between items-center w-full z-40 bg-gradient-to-br from-green-700 to-green-500 px-4 md:px-8 py-3 text-white font-bold text-xl"
      >
      ...
      {openGroupInfo && (
        <Card
          ref={cardRef}
          className="absolute z-40 flex top-16 left-0 cursor-default w-96 max-h-96"
        >
          <div className="bg-gray-100">
            <Box
              onClick={openGroupOverviewHandler}
              className="flex justify-between gap-2 p-2 hover:bg-gray-200"
            >
              <InfoOutlinedIcon className="p-0.5" />
              <Typography className="font-semibold">Overview</Typography>
            </Box>
            <Box
              onClick={openGroupMembersHandler}
              className="flex justify-between gap-2 p-2 hover:bg-gray-200"
            >
